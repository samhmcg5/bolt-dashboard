...
// QMath.h for Qt4.8 does not include these functions
// so I have defined them here. SHMV 20160316
...
QcColorBand *QcGaugeWidget::addColorBand(float position, int one , int two , int three)
{
    QcColorBand * item = new QcColorBand(this, one, two, three);
    item->setPosition(position);
    mItems.append(item);
    return item;
}
...
QPointF QcItem::getPoint(float deg,const QRectF &tmpRect)
{
    ...
    float xx=cos(DegreesToRadians(deg))*r;
    float yy=sin(DegreesToRadians(deg))*r;
    QPointF pt;
...
float QcItem::getAngle(const QPointF&pt, const QRectF &tmpRect)
{
    ...
    return RadiansToDegrees( atan2(yy,xx));
}
...
QcColorBand::QcColorBand(QObject *parent,int one , int two , int three ) :
    QcScaleItem(parent) {
    QColor tmpColor;
...
void QcNeedleItem::setCurrentValue(float value)
{
    ...
    if(mLabel!=0)
        mLabel->setText(QString::number(int(mCurrentValue*1000)),false);
